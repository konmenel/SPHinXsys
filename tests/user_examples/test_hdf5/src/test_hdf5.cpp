...
int main(int argc, char *argv[])
{
	...
	//the walls
	WallBoundary wall_boundary(system, "Wall", ch_system);
	wall_boundary.setChronoBody(createChWalls());
	// create solid particles
	SolidParticles wall_particles(wall_boundary);

	//the boulder
	Boulder boulder(system, "Boulder", ch_system);
	boulder.setChronoBody(createChBoulder());
	// create solid particles
	...
	ch_system.SetTimestepperType(chrono::ChTimestepper::Type::EULER_IMPLICIT_LINEARIZED);
	ch_system.SetSolverType(chrono::ChSolver::Type::PSOR);
	ch_system.SetSolverMaxIterations(50);
	ch_system.Set_G_acc(ChVector<>(0.0, 0.0, -gravity_g));

	auto boulder_ch = boulder.getChBody();
	BoulderSystemForChrono boulder_for_chrono(boulder, "Boulder_chrono", boulder.body_shape_);
	ConstrainSolidBodyPartByChrono boulder_constain(boulder, boulder_for_chrono);
	TotalForceOnSolidBodyPartForChrono force_on_boulder(boulder, boulder_for_chrono);
	...
	BodyStatesRecordingToLegacyVtk write_body_states_vtk(in_output, system.real_bodies_);
	SaveStateToH5 write_body_states(in_output);
	write_body_states.writeToFile();
	write_body_states_vtk.writeToFile(0);
	...
	// TODO: Chrono body is not updated!!
	...
	while (sim_time < end_time) {
		...
		while (integration_time < out_dt) {
			...
			if (number_of_iterations % report_steps == 0) {
				fcout << std::fixed << std::setprecision(9)
				<< "Step=" << std::setw(4) << number_of_iterations
				<< "\tTime=" << std::setw(8) << sim_time << endl;
			}
		...
		write_body_states.writeToFile();
		write_body_states_vtk.writeToFile(number_of_iterations);
